			...
			<form id="FormTrabalho" enctype="multipart/form-data">
			  ...
			  <div class="form-check">
				<label class="form-check-label">
				  ...
				  Eu, '.$arquivos->nome.', aceito ceder ao XVI ENEPET o direito de reprodução do trabalho enviado das maneiras que forem convenientes ao evento.
				</label>
			  </div>
...
else if($acao == 'uploadTrabalho')
{
	...
	if(!empty($_POST['titulo']) && 
		...
		!empty($_FILES['termoPDF']['tmp_name']) && 
		!empty($_FILES['arquivoPDF']['tmp_name']))
	{

		//SE TIVER ARQUIVO
		$ccpf = validaCPF(retirar_letras_e_pontos($_POST['cpf']))?retirar_letras_e_pontos($_POST['cpf']):'';
		$rs = $con->prepare("SELECT * FROM inscricoes WHERE cpf = ?");
		$rs->bindParam(1, $ccpf);
		...
		if($rs->execute()){
			if($rs->rowCount() == 0 || $rs->rowCount()>=2){
				...
				$caminho_termo = $_FILES['termoPDF']['tmp_name'];
				$caminho2_termo = $_FILES['termoPDF']['name'];
				$infos_termo = pathinfo($caminho2_termo);
				$nome_arquivo_termo = 'termo_'. $novo_nome.'.'.$infos_termo['extension'];
				//$nome_arquivo_termo = str_replace(" ", "_", $nome_arquivo_termo);
				$link['termo'] = $nome_arquivo_termo;
				$arquivo_termo = $diretorio.$nome_arquivo_termo;
				//move_uploaded_file($value, $nome_original)
				$links = json_encode($link);
				
				if(!move_uploaded_file($caminho_termo, $arquivo_termo)){
					//@unlink($arquivo_resumo);
				...
				$stmt->bindParam(6,$arquivos->estado); 
				$stmt->bindParam(7,$arquivos->pet); 
